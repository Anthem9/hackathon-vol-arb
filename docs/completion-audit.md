...
| Polymarket readiness | Public CLOB reachable; account/readiness/order-preview/cancel-preview/order-execute/cancel-execute implemented; CLI helper checks wallet/private-key/L2 credential readiness and can explicitly create or derive L2 credentials without printing secrets; authenticated collateral balance, allowance, and open-order reads are proven with official CLOB L2 signing; live execution is blocked by default | Authenticated reads complete; guarded live execution controls complete; live trading intentionally disabled until operator approval |
| Secret safety | `.dockerignore`, `.gitignore`, `scripts/secret-scan.mjs`, CI workflow, `npm run secret:scan` passes | Complete |
| Browser smoke | Production-like web returns HTTP 200 at `http://localhost:3001`; the old local Playwright smoke spec is not present in the repo | Basic service smoke complete; restore a committed Playwright smoke before final UI signoff |
...
- Production-like web smoke after rebuild: `curl -fsS -o /dev/null -w '%{http_code} %{content_type}\n' 'http://localhost:3001/'` returned `200 text/html; charset=utf-8`; the old `dashboard-smoke.spec.js` / `empty.config.js` files are not present in the repo, so a committed Playwright smoke should be restored before final UI signoff.
